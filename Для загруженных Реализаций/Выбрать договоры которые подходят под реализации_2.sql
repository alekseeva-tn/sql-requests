SELECT D.Date, D.DocumentNumber, D.AmountID, C.StartDate, C.DueDate, C.ContractNumber, C.Amount
FROM tbl_Document D, tbl_Contract C
WHERE 
	D.AccountID = C.CustomerID AND
	D.OwnerID = C.OwnerID AND
	(CONVERT(char(10), D.Date, 102) >=  CONVERT(char(10), C.StartDate, 102)) AND
	/*(	
	(CONVERT(char(10), D.Date, 102) <= CONVERT(char(10), C.DueDate, 102)) OR
	(Ñ.DueDate = null)
	)
	AND*/
	(D.ContractID is NULL) AND
	(C.StartDate is not NULL) AND
	(D.AmountID = C.Amount)
	--(Ñ.DueDate is null)
ORDER BY D.Date, D.DocumentNumber
